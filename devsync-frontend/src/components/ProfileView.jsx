...
const ProfileView = ({ user, createAlert }) => {
  ...
  const handleSkillsSubmit = () => {
    ...
    if (skillString === user.skill || skillString === '' || skillString === null) {
      return;
    }
    axios
    .put(BASE_URL + `/api/user/${currUser.userId}/skill`, skillString, {
      headers: { Authorization: `Bearer ${currUser.jwt}`, 'Content-Type': 'text/plain' }
    })
    .then(res => {
    ...
    <>
      <div className="flex flex-col p-1">
        ...
        <div className="flex flex-row justify-between border-b pb-2 mt-4">
          ...
          <button className="bg-zinc-700 rounded-md px-4 ml-4 hover:cursor-pointer py-1 hover:bg-blue-500" onClick={handleSkillsSubmit}>
            Save
          </button>
          </div>
